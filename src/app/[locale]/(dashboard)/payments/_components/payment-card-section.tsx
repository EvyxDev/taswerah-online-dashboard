...
const Card = ({ title, value, unit, iconSrc }: CardProps) => {
  return (
    <div
      className={
        "bg-white flex flex-col justify-between shadow-md border-black border-[12px] p-4 2xl:p-6 rounded-3xl h-40 sm:h-44 md:h-48 lg:h-40 2xl:h-48"
      }
    >
...
export default function PaymentCardSection({
  ...
  const totalSales = (data.monthly_payments || []).reduce((sum, item) => {
    const numeric =
  ...
  const printedPhotos =
    (data.photo_stats?.print || 0) + (data.photo_stats?.print_and_send || 0);
  const staffCount = data.staff?.length || 0;
  const activeBooths = 0;

  const cardData = [
    {
      title: t("dashboard.totalSales"),
      value: Number.isFinite(totalSales) ? totalSales.toFixed(0) : 0,
